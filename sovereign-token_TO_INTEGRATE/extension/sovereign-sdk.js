...
class TokenVisualEngine {
    ...
    _drawBackground() {
        ...
        const grad = ctx.createRadialGradient(cx, cy, 0, cx, cy, FRAME_SIZE * 0.8);
        const [h1, h2, h3] = this.palette;
        grad.addColorStop(0, `hsla(${h1},80%,12%,1)`);
        grad.addColorStop(0.5, `hsla(${h2},70%,8%,1)`);
        grad.addColorStop(1, `hsla(${h3},60%,4%,1)`);
        ctx.fillStyle = grad;
        ctx.fillRect(0, 0, FRAME_SIZE, FRAME_SIZE);
    }

    _drawParticles(f) {
        const ctx = this.ctx, [h1] = this.palette, norm = f / FRAME_COUNT;
        for (const p of this.particles) {
            const alpha = 0.4 + 0.4 * Math.sin(p.life * Math.PI * 2 + norm * Math.PI);
            const hue = h1 + p.hueOffset + norm * 30;
            const lum = 55 + 30 * Math.sin(p.phase + p.life * 5 + norm * Math.PI * 2);
            ctx.beginPath();
            ctx.arc(p.x, p.y, p.size, 0, Math.PI * 2);
            ctx.fillStyle = `hsla(${hue},90%,${lum}%,${alpha})`;
            ctx.fill();
        }
    }
    ...
    _drawSigil(f) {
        ...
        const pulse = 1 + 0.06 * Math.sin(norm * Math.PI * 2);
        const spin = norm * Math.PI * 0.4;
        ctx.save();
        ctx.translate(cx, cy);
        ctx.rotate(spin);
        ctx.scale(pulse, pulse);
        const rg = ctx.createRadialGradient(0, 0, 28, 0, 0, 38);
        rg.addColorStop(0, `hsla(${h1},90%,70%,0)`);
        rg.addColorStop(0.5, `hsla(${h1},90%,70%,0.6)`);
        rg.addColorStop(1, `hsla(${h1},90%,70%,0)`);
        ctx.beginPath(); ctx.arc(0, 0, 33, 0, Math.PI * 2);
        ctx.strokeStyle = rg; ctx.lineWidth = 1.2; ctx.stroke();
        for (const arm of this.sigilPoints) {
            ...
            ctx.strokeStyle = `hsla(${h1 + norm * 40},85%,65%,0.8)`;
            ctx.lineWidth = 0.8; ctx.stroke();
        }
        const cg = ctx.createRadialGradient(0, 0, 0, 0, 0, 8);
        cg.addColorStop(0, `hsla(${h2},100%,90%,0.9)`);
        cg.addColorStop(1, `hsla(${h2},90%,60%,0)`);
...
export class SovereignSDK {
    ...
    static _derivePalette(seed) {
        ...
        return [h1, (h1 + 137) % 360, (h1 + 250) % 360];
    }
