...
export default function FundingPanel({ article, onFunded }) {
  ...
  const initiatePayment = async (dollarAmount) => {
    ...
    try {
      const { clientSecret: secret, amountCharged } =
        await createPaymentIntent(article.id, parsed);
  ...
  const handleSuccess = (amountDollars) => {
      ...
      {remaining !== null && (
        <div>
          <div style={{
            ...
            <div style={{
              width: `${Math.min(100, (article.amount_raised / article.funding_goal) * 100)}%`,
              height: "100%",
          ...
          <p style={{ fontSize: 12, color: "#6B5E52", marginBottom: 16 }}>
            ${article.amount_raised} raised · ${remaining} remaining
          </p>
        </div>
      )}
        ...
        <>
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 10, flexWrap: "wrap" }}>
            ...
            {remaining !== null && remaining > 0 && (
              ...
              >
                Fund all ${remaining} — free it for everyone
              </button>
            )}
          </div>
        </>
